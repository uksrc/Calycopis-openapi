...
schema into a single YAML file at `schema/build/execution-broker-1.0.3.yaml`.
...
python isobeon/schema-processor.py \
    ...
    schema/build/execution-broker-1.0.3.yaml
```
...
this artifact (`net.ivoa.calycopis:calycopis-spring:1.0.3-SNAPSHOT`).
...
java -jar /opt/openapi-generator/openapi-generator-cli-7.18.0.jar \
    ...
    --input-spec schema/build/execution-broker-1.0.3.yaml \
...
The current schema version is `1.0.3`. This version string appears in:

* `bin/buildscripts.sh` (`schemaversion` variable)
* `project.properties`
* The Maven POM files (`<version>1.0.3-SNAPSHOT</version>`)
* The generated Python package (`packageVersion`)
* The processed schema output filename (`execution-broker-1.0.3.yaml`)
